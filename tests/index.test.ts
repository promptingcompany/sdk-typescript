...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Tpc({ logger: logger, logLevel: 'debug', bearerToken: 'My Bearer Token' });
    ...
    test('default logLevel is warn', async () => {
      const client = new Tpc({ bearerToken: 'My Bearer Token' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Tpc({ logger: logger, logLevel: 'info', bearerToken: 'My Bearer Token' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new Tpc({ logger: logger, bearerToken: 'My Bearer Token' });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['TPC_LOG'] = 'debug';
      const client = new Tpc({ logger: logger, logLevel: 'off', bearerToken: 'My Bearer Token' });
  ...
  describe('baseUrl', () => {
    ...
    test('explicit option', () => {
      const client = new Tpc({ baseURL: 'https://example.com', bearerToken: 'My Bearer Token' });
    ...
    test('env variable with environment', () => {
      ...
      expect(
        () => new Tpc({ bearerToken: 'My Bearer Token', environment: 'production' }),
      ...
      const client = new Tpc({ bearerToken: 'My Bearer Token', baseURL: null, environment: 'production' });
      expect(client.baseURL).toEqual('https://app.promptingcompany.com');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new Tpc({ bearerToken: 'My Bearer Token', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Tpc({ maxRetries: 4, bearerToken: 'My Bearer Token' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Tpc({ bearerToken: 'My Bearer Token' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['TPC_BEARER_TOKEN'] = 'My Bearer Token';
    const client = new Tpc();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['TPC_BEARER_TOKEN'] = 'another My Bearer Token';
    const client = new Tpc({ bearerToken: 'My Bearer Token' });
    expect(client.bearerToken).toBe('My Bearer Token');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Tpc({ bearerToken: 'My Bearer Token', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Tpc({ bearerToken: 'My Bearer Token', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Tpc({ bearerToken: 'My Bearer Token', fetch: testFetch });
